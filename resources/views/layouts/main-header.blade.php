								...
								<div class="dropdown-menu">
									<div class="menu-header-content bg-primary text-right">
										<div class="d-flex">
											...
											<a href="{{ url('mark_all') }}"><span class="badge badge-pill badge-warning mr-auto my-auto float-left">قراءة الكل</span></a>
										</div>
										<p id="notification_count" class="dropdown-title-text subtext mb-0 text-white op-6 pb-0 tx-12 ">لديك  ({{ Auth::user()->unreadNotifications->count() }}) اشعارات غير مقروءة </p>
									</div>
									<div class="main-notification-list Notification-scroll" id="notification_body">
                                        @foreach (Auth::user()->unreadNotifications as $item)


										<a class="d-flex p-3 border-bottom" href="{{ url('invoicedetalls') }}/{{$item->data['invoice_id'] }}">
											<div class="notifyimg bg-pink">
												<i class="la la-file-alt text-white"></i>
											</div>
											<div class="mr-3">
                                                <div class="notification-subtext">{{ $item->data['title'] }}</div>
												<h5 class="notification-label mb-1">{{ $item->data['userCearte'] }}</h5>

                                                <small class="text_warning">{{$item->created_at  }}</small>
											</div>
											<div class="mr-auto" >
